...
with st.sidebar:
    ...
    with st.popover("💬 Trợ lý ảo ", use_container_width=True):
        ...
        if prompt := st.chat_input("Hỏi AI về bài toán này...", key="chat_popover_input"):
            # 1. Hiển thị câu hỏi người dùng
            st.session_state.chat_history.append({"role": "user", "content": prompt})
            with messages_container:
                st.chat_message("user").write(prompt)
            
            # 2. LẤY NGỮ CẢNH (CONTEXT) TỪ BỘ NHỚ
            curr_problem = st.session_state.get('problem_text_input', "Người dùng chưa nhập đề bài")
            curr_code = st.session_state.get('cpp_code_content', "Người dùng chưa viết code")
            
            # 3. TẠO PROMPT KÈM NGỮ CẢNH
            full_prompt = f"""
            Role: C++ Tutor & Assistant.
            ...
            INSTRUCTION: Answer the user's question based on the Context Information provided above. Answer in VIETNAMESE.
            """

            # 4. Gửi cho AI và hiển thị câu trả lời
            with messages_container:
                with st.chat_message("assistant"):
                    res_box = st.empty(); full_res = ""
                    # Lưu ý: Truyền full_prompt (có ngữ cảnh) thay vì chỉ prompt
                    for chunk in stream_ai_response(full_prompt):
...
# === KHUNG NHẬP LIỆU CHÍNH (FIX LỖI MẤT CHỮ CHẠY) ===
with st.container(border=True):
