...
<style>
    ...
    /* Style cho nút Paste - Cố gắng làm cho nó đẹp hơn */
    iframe[title="st_img_pastebutton.paste"] { min-height: 45px !important; }
...
def handle_solve_process():
    ...
    if not final_text and not cur_img:
        st.toast("Thiếu đề bài (Text/Ảnh)!", icon="⚠️"); return

    with st.spinner("🧠 Đang xây dựng Logic Test..."):
...
with st.sidebar:
    ...
    with st.popover("💬 Trợ lý ảo (Click để chat)", use_container_width=True):
        ...
        with col_c1:
            if HAS_PASTE_LIB:
                try: 
                    # Thêm try-except để tránh crash nếu browser chặn
                    chat_p = paste(label="📋 Dán (Ctrl+V)", key="chat_paste_btn")
                    if chat_p: st.session_state['chat_pasted_image'] = Image.open(io.BytesIO(chat_p))
                except Exception as e: st.caption(f"⚠️ Lỗi Paste: {e}")
            else: st.caption("⚠️ Thiếu thư viện")
            
        with col_c2:
             up_chat = st.file_uploader("📂 Upload", type=["png","jpg"], key="chat_up", label_visibility="collapsed")
             if up_chat: st.session_state['chat_pasted_image'] = Image.open(up_chat)

        if st.session_state['chat_pasted_image']:
            st.image(st.session_state['chat_pasted_image'], width=150)
            if st.button("❌ Xóa", key="del_chat_img"): 
        ...
        if prompt := st.chat_input("Hỏi AI...", key="chat_popover_input"):
            ...
            full_prompt = f"Role: C++ Tutor. Context: Problem: {curr_prob}. Code: {curr_code}. User: {prompt}. Answer in VN."
...
<div class="ticker-wrap">
    <div class="ticker">
        ...
        <div class="ticker-item">⚡ TIPS: Bạn có thể DÁN ẢNH trực tiếp ở khung bên phải</div>
...
# === KHUNG NHẬP LIỆU CHÍNH (ĐÃ FIX LỖI HIỂN THỊ) ===
with st.container(border=True):
    ...
    with c2:
        ...
        tab_up, tab_paste = st.tabs(["📁 File", "📋 Paste"])
        ...
        with tab_paste:
            if HAS_PASTE_LIB:
                try:
                    # Xử lý lỗi cụ thể tại đây để không hiện hộp đỏ
                    ...
                    # Thay vì hiện st.error, ta hiện cảnh báo nhẹ nhàng
    ...
    with b1:
        st.button("🚀 NẠP ĐỀ BÀI", type="primary", use_container_width=True, on_click=handle_solve_process)
    with b2:
...
if st.session_state.get('failed_cases'):
    ...
    if st.button("🤖 Sửa code giúp tôi", type="primary"):
